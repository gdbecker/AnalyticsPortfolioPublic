...
function ProjectCard({ id, img, title, code_url, types }) {
  ...
  function getLabel(type) {
    return (
      <p className="px-2 my-1 rounded-sm font-interBold text-sm text-right overflow-hidden lg:text-[0.75rem]">
        <span className="h-6 w-auto inline-block [&>svg]:h-full [&>svg]:w-auto">
          {getIcon('toolLabel', type)}
        </span>
      </p>
    );
  }
      ...
      <div className="flex-auto grid grid-cols-1 px-5 py-3 pb-1 text-gray">
        ...
        <div className="grid grid-cols-2 items-end justify-between py-3 font-interRegular text-md lg:text-sm">
          ...
          <div className="flex flex-col items-end justify-end">
            <div className="flex flex-wrap justify-end w-8/12 md:w-fit">
